...
class BlockEncoder {
    ...
    encodeItem (item, res, lists) {
        ...
        case 'prim':
                let sym;
                Object.keys(this.BLOCK_SYMS).forEach(key => {
                    if (item[1].includes(key)) {
                        sym = this.BLOCK_SYMS[key];
                    }
                });
                res.push(sym.byte);
                break;
    ...
    encodeBlock (block, subs, targets) {
        ...
        if (block._block.opcode === 'light_whenStarted') hat.push(8);
        if (block._block.opcode === 'motor_whenStarted') hat.push(8);
